...
//
// fml 线段结果的车道线聚类，与 fml.hpp 配套使用：
//   fml_cluster_lines   将检测线段按车道线归属聚类，每条车道线输出
//                       "中心点链 + 曲线拟合描述"（算法与实现见 cluster.cpp）
//   fml_draw_clusters   聚类结果绘制（链视图/拟合视图/簇切分视图，只画不弹窗）
//   fml_lane_scorer_*   车道线打分器权重（内置默认 / 文本文件加载 / 自定义）
//
// 编译器需求：同 fml.hpp；依赖：opencv4.x+
//
// 用法：
//   cv::Mat gray = ...;                                   // 8 位单通道灰度图
//   std::vector<cv::Vec4f> lines;
//   std::vector<float> confs;
//   fml_detect_elsed(gray, ep, lines, &confs);            // 见 fml.hpp
//   fml_cluster_params cp;                                // 默认值即推荐值
//   std::vector<FmlSegCluster> cls;
//   int nv = fml_cluster_lines(lines, confs, gray.cols, gray.rows, cls, cp);
//   cv::Mat canvas = ...;                                 // BGR 显示图
//   fml_draw_clusters(canvas, cls, canvas.cols/(double)gray.cols,
//                     canvas.rows/(double)gray.rows, 12, FML_CLUSTER_DRAW_CHAINS);
//
// cls[i] 按 score 降序；centers 为该车道线中心点链（按 y 降序，自底向上，
//   每内点节点一个点）；score 综合链长/端到端延展/平均置信度，越大越可靠
//   （lane_scorer 开启时由打分器重排）。valid=false 表示未达 min_length、
//   居顶部噪声带或链呈锯齿形态，得分已打折；a2/a1/a0 与 ra/rb/rc、rms
//   均为中心链的描述性拟合，不参与任何判据
//
// 输入坐标系：lines/confs 为 fml 在 width×height 分辨率下的输出，width/height
//   须与检测分辨率一致（绘制缩放由 fml_draw_clusters 的 scale 参数负责）
// 参数缺省值按 320*240 工作分辨率标定，其他分辨率按比例设置
// 确定性：同输入同输出；无内部多线程，各线程可用独立实例参数并行调用
// 
// 注意：
// 1 算法默认输出带旋转自由度的二次曲线作为拟合结果，注意结果使用。也支持拟合直线。
// 2 打分体系中，只用默认即可，如果你确实追求这个程序的上限，请自行训练基于学习的方法。
//   训练脚本在utils中。这个方法是数据驱动的，有默认权重，但是是用我自己的数据集训练的。
// 3 使用的时候一定认真看注释。不懂的问ai也好过自己随便用。
// 4 这个程序仅仅是拓展模块，属于个人思路，可以用也可以不用，即使使用也推荐自己再做一轮筛选或者仔细调参。
//   fml是核心模块。推荐按自己的需求搭配使用。
// 5 不理解的地方欢迎询问，这个算法确实比较复杂 qq439887968
...
struct FmlSegCluster
{
    ...
    float conf = 0.f;                   ///< 途经节点合并置信度的均值
                                        ///< （原始 -log10 NFA，仅参考）
    bool valid = false;                 ///< 链长达标、非顶部噪声带且非
                                        ///< 锯齿链（双缘混线拒绝）；
                                        ///< 锯齿=相邻窗口中心差分
                                        ///< 反号且双侧幅值>4px 的
                                        ///< 折返连续 ≥3 处（左右左
...
///
/// 特征布局（24 维，与训练数据一致的固定布局，索引不可变更）：
///  0 npts, 1 length, 2 score(加法口径), 3 conf, 4 sumc=conf·npts, 5 theta,
